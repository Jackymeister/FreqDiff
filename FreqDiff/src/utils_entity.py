...
def _resolve_eval_hits(default_hits):
    """Resolve hit@k list from env var while keeping legacy defaults.

    DIFFUTKG_EVAL_HITS supports comma-separated integers, e.g. "4,13".
    """
    raw_hits = os.getenv("DIFFUTKG_EVAL_HITS", "").strip()
    if not raw_hits:
        return default_hits

    parsed_hits = []
    for token in raw_hits.split(","):
        token = token.strip()
        if not token:
            continue
        try:
    ...
    dedup_hits = []
    seen = set()
    for hit_k in parsed_hits:
        if hit_k in seen:
            continue
        seen.add(hit_k)
        dedup_hits.append(hit_k)
    return dedup_hits
...
def cal_ranks(rank_list, mode):
    ...
    hits_log = []
    hits = _resolve_eval_hits([1, 3, 10])
...
def load_data(dataset):
    if dataset in ['EG', 'IS', 'IR', 'ICEWS14', 'ICEWS14_REL', 'ICEWS05-15', 'ICEWS05_15', 'ICEWS18', 'WIKI', 'GDELT', 'YAGO']:
        if dataset == 'ICEWS05_15':
            dataset = 'ICEWS05-15'
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        data_dir = os.path.join(project_root, "data")
        return knwlgrh.load_from_local(data_dir, dataset)
    else:
