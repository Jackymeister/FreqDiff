...
import torch
import math
from diffusion import DiffuRec
...
class Att_Diffuse_model(nn.Module):
    def __init__(self, diffu, args, encoder_name, num_ents, num_rels,
        ...
        self.use_fft_loss = getattr(args, "use_fft_loss", False)
        self.fft_loss_weight = getattr(args, "fft_loss_weight", 0.01)
        self.fft_loss_type = getattr(args, "fft_loss_type", "mix_loss")
        self.fft_loss_source = getattr(args, "fft_loss_source", "fused_rep")
        self.fft_loss_detach_target = getattr(args, "fft_loss_detach_target", True)
        self.fft_loss_history_only = getattr(args, "fft_loss_history_only", True)
        self.fft_loss_mode = getattr(args, "fft_loss_mode", "sequence")
        self.freq_aux_loss = None
    ...
    def fft_base_loss(self, pred, true, reduction="none"):
        if self.fft_loss_type == "l1":
            return F.l1_loss(pred, true, reduction=reduction)
        elif self.fft_loss_type == "l2":
            return F.mse_loss(pred, true, reduction=reduction)
        elif self.fft_loss_type == "SmoothL1Loss":
            return F.smooth_l1_loss(pred, true, reduction=reduction)
        elif self.fft_loss_type == "mix_loss":
            l1 = F.l1_loss(pred, true, reduction=reduction)
            l2 = F.mse_loss(pred, true, reduction=reduction)
            return 0.5 * l1 + 0.5 * l2
        else:
            raise ValueError("invalid fft_loss_type {}".format(self.fft_loss_type))

    def fft_loss(self, model_out, target, mask_seq=None):
        if self.fft_loss_detach_target:
            target = target.detach()
        if mask_seq is not None:
            model_out = model_out * mask_seq.unsqueeze(-1)
            target = target * mask_seq.unsqueeze(-1)
        ...
        fourier_loss = (
            self.fft_base_loss(torch.real(fft1), torch.real(fft2), reduction="none")
            + self.fft_base_loss(torch.imag(fft1), torch.imag(fft2), reduction="none")
        )

        if mask_seq is not None:
            fourier_loss = fourier_loss * mask_seq.unsqueeze(-1)
            denom = mask_seq.sum() * fourier_loss.size(-1)
            return fourier_loss.sum() / denom.clamp_min(1.0)
        return fourier_loss.mean()

    def fft_loss_query_object(self, model_out, target):
        if self.fft_loss_detach_target:
            target = target.detach()

        query_pred = model_out[:, -1, :]
        query_target = target[:, -1, :]

        fft1 = torch.fft.rfft(query_pred.float(), dim=-1, norm="forward")
        fft2 = torch.fft.rfft(query_target.float(), dim=-1, norm="forward")
        fourier_loss = (
            self.fft_base_loss(torch.real(fft1), torch.real(fft2), reduction="none")
            + self.fft_base_loss(torch.imag(fft1), torch.imag(fft2), reduction="none")
        )
    ...
    def compute_freq_aux_loss(self, target_sequence, mask_seq):
        aux = getattr(self.diffu.xstart_model, "last_aux", {})
        model_out = aux.get(self.fft_loss_source, None)
        if model_out is None:
            return None
        if self.fft_loss_mode == "query_object":
            return self.fft_loss_query_object(model_out, target_sequence)
        if self.fft_loss_history_only and model_out.size(1) > 1:
            model_out = model_out[:, :-1, :]
            target_sequence = target_sequence[:, :-1, :]
            if mask_seq is not None:
                mask_seq = mask_seq[:, :-1]
        return self.fft_loss(model_out, target_sequence, mask_seq)
    ...
    def loss_diffu_ce(self, rep_diffu, labels,history_tail_seq=None, one_hot_tail_seq=None,true_triples=None,mask_seq=None,emb_ent=None):
        ...
        total_loss = self.loss_ce(scores, labels.squeeze(-1)) + loss
        if self.use_fft_loss and self.freq_aux_loss is not None:
            total_loss = total_loss + self.fft_loss_weight * self.freq_aux_loss
        return total_loss
    ...
    def forward(self, sequence, tag, train_flag=True,use_cuda=True,history_tail_seq = None,seen_before_label=None,static_graph=None):
        ...
        if train_flag: ### 训练过程
            ...
            clean_object_embeddings = torch.concat([object_embeddings,query_object3],dim=1)
            target_inter_embeddings = clean_object_embeddings + time_embeddings_re + relation_embeddings
            target_inter_embeddings = self.LayerNorm(target_inter_embeddings)
            ...
            rep_diffu = self.diffu_pre(
                inter_embeddings_drop,
                inter_embeddings_drop[:,-1,:],
                initial_h,
                mask_seq,
                t,
                query_sub3=query_rel3.squeeze(1)
            )
            if self.use_fft_loss:
                self.freq_aux_loss = self.compute_freq_aux_loss(target_inter_embeddings, mask_seq)
